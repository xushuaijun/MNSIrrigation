...
public class DeleteLandBiz extends AbstractNetBiz {


    private Context context;
    private NetCallBack<PileDeleteResult> netcallback;

    public DeleteLandBiz(){
        super();

    }

    public void deland(Context context,String landnum ,
        ...
        RequestParams params=new RequestParams(Const.land_delete);
        params.addBodyParameter("token", MyApplication.mApplication.getToken());
        params.addBodyParameter("landnum",landnum);
        httpbiz.connectNet(HttpBiz.POST, params);
    }
    @Override
    public void netSuccess(String url, String result) {
        if(!TextUtils.isEmpty(url) && Const.land_delete.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)) {
