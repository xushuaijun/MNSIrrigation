...
public class DeleteSoilMoistureBiz extends AbstractNetBiz {


    private Context context;
    private NetCallBack<PileDeleteResult> netcallback;

    public DeleteSoilMoistureBiz(){
        super();

    }

    public void delSoilMois(Context context,String soilExplorerNum ,
        ...
        RequestParams params=new RequestParams(Const.soil_moisture_delete);
        params.addBodyParameter("token", MyApplication.mApplication.getToken());
        params.addBodyParameter("soilExplorerNum",soilExplorerNum);
        httpbiz.connectNet(HttpBiz.POST, params);
    }
    @Override
    public void netSuccess(String url, String result) {
        if(!TextUtils.isEmpty(url) && Const.soil_moisture_delete.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)) {
            ...
            LogUtil.e("删除墒情", "解析后" + (datamessage.toString()));
