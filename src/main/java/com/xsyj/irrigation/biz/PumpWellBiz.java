 ...
 */
public class PumpWellBiz extends AbstractNetBiz{

    private Context context;
    private NetCallBack<CommonData<List<PumpWell>>> netcallback;
    ...
    public void pumpwell_getdata(Context context,String pumpname,String start_id,String page_size,NetCallBack<CommonData<List<PumpWell>>> netcallback){
        ...
        RequestParams params=new RequestParams(Const.pumpinfo_list);
        ...
        params.addBodyParameter("pumpname",pumpname);
    ...
    public void netSuccess(String url, String result) {
        ...
        if(!TextUtils.isEmpty(url) && Const.pumpinfo_list.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)){

            // 获取用户类型
            Type type= new TypeToken<CommonData<List<PumpWell>>>(){}.getType();
            LogUtil.e("机井基本信息列表", "解析前");
            CommonData<List<PumpWell>> datamessage = JsonParserUtil.getObject(result, type);
