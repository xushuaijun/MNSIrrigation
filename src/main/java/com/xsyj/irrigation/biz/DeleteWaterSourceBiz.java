...
public class DeleteWaterSourceBiz extends AbstractNetBiz {


    private Context context;
    private NetCallBack<PileDeleteResult> netcallback;

    public DeleteWaterSourceBiz(){
        super();

    }

    public void delWaterSource(Context context,int id,
        ...
        RequestParams params=new RequestParams(Const.delete_pumphouse);
    ...
    public void netSuccess(String url, String result) {
        if(!TextUtils.isEmpty(url) && Const.delete_pumphouse.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)) {
