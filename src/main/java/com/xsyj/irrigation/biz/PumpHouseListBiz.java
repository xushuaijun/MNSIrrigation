...
public class PumpHouseListBiz extends AbstractNetBiz {

    private Context context;
    private NetCallBack<CommonData<List<PumpHouseList>>> netCallBack;

    public PumpHouseListBiz(){
        super();

    }

    public void getPumpHouseList(Context context, String token, String areaId, String areaLevel, NetCallBack<CommonData<List<PumpHouseList>>> netCallBack){
        ...
        RequestParams params=new RequestParams(Const.get_PumpHouseList);
...
//        String areaId = MyApplication.mApplication.getAreaId();
    ...
    public void netSuccess(String url, String result) {
        ...
        Type type= new TypeToken<CommonData<List<PumpHouseList>>>(){}.getType();
        LogUtil.e("出地桩基本信息列表", "解析前");
        CommonData<List<PumpHouseList>> datamessage = JsonParserUtil.getObject(result, type);
        if (datamessage != null){
