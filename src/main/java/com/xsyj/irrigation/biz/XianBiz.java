...
//import com.xsyj.myapplication.entity.Xian;
 ...
 */
public class XianBiz extends AbstractNetBiz {


    private Context context;
    private NetCallBack<CommonDataE<CommInData<List<Xian>>>> netcallback;

    public XianBiz() {
    }

    public void getdata(Context context,String type,String shino,String ADDVCD,NetCallBack<CommonDataE<CommInData<List<Xian>>>> netcallback) {
    ...
    public void netSuccess(String url, String result) {
        LogUtil.e("++++++县列表++++++", result);
        if(!TextUtils.isEmpty(url) && Const.XIAN_LISt_URL.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)){
            // 获取用户类型
            Type type= new TypeToken<CommonDataE<CommInData<List<Xian>>>>(){}.getType();
            // 根据类型返回实体类
            CommonDataE<CommInData<List<Xian>>>  datamessage= JsonParserUtil.getObject(result, type);
            ...
            if(datamessage!=null){
                switch (datamessage.getData().getS()){
                    ...
                    case 500:  // 返回失败
                        ...
                        break;
                    default:
                        ToastUtil.toast(context, "返回县列表");
                        LogUtil.e("返回县列表","既不是500，也不是200");
                        break;
                }
            }else{
                LogUtil.e("返回县列表解析失败",result);
            }
        }else{
            LogUtil.e("返回县列表结果为空",result);
        }
    }
