 ...
 */
public class GPRSBiz extends AbstractNetBiz{

    private Context context;
    private NetCallBack<CommonData<List<GetGPRSResult>>> netcallback;
    ...
    public void gprs_getdata(Context context,String landname,String start_id,String page_size,
                             NetCallBack<CommonData<List<GetGPRSResult>>> netcallback){
        ...
        RequestParams params=new RequestParams(Const.gprs_list);
        ...
        params.addBodyParameter("gprsName",landname);
    ...
    public void netSuccess(String url, String result) {
        ...
        if(!TextUtils.isEmpty(url) && Const.gprs_list.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)){

            // 获取用户类型
            Type type= new TypeToken<CommonData<List<GetGPRSResult>>>(){}.getType();
            LogUtil.e("地块信息列表", "解析前");
            CommonData<List<GetGPRSResult>> datamessage = JsonParserUtil.getObject(result, type);
