 ...
 */
public class WaterSourceBiz extends AbstractNetBiz{

    private Context context;
    private NetCallBack<CommonData<List<WaterSource>>> netcallback;

    public WaterSourceBiz(){
        super();

    }

    public void waterSource_getdata(Context context,String pumphousenum,String start_id,String page_size,
                                    NetCallBack<CommonData<List<WaterSource>>> netcallback){
        ...
        RequestParams params=new RequestParams(Const.pumpHouse_list);
...
//        params.addBodyParameter("page_size", page_size);
        LogUtil.e("机井参数", start_id + "/" + page_size);
    ...
    public void netSuccess(String url, String result) {
        ...
        LogUtil.e("机井基本信息列表", result);
        if(!TextUtils.isEmpty(url) && Const.pumpHouse_list.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)){

            // 获取用户类型
            Type type= new TypeToken<CommonData<List<WaterSource>>>(){}.getType();
            LogUtil.e("机井基本信息列表", "解析前");
            CommonData<List<WaterSource>> datamessage = JsonParserUtil.getObject(result, type);

            LogUtil.e("机井基本信息列表", "解析后"+(datamessage.toString()));
            //判断解析数据是否为空
            if(datamessage!=null){
