 ...
 */
public class TaskToViewBiz extends AbstractNetBiz{

    private Context context;
    private NetCallBack<CommonData<List<TaskToView>>> netcallback;

    public TaskToViewBiz(){
        super();

    }

    public void getInfo(Context context, String start_id, int tasktype, int state, String tapNum,
                        String netGateCode, String maxAddTime, String minAddTime,
                        NetCallBack<CommonData<List<TaskToView>>> netcallback){
        ...
        RequestParams params=new RequestParams(Const.TASKTOVIEW);
        ...
        params.addBodyParameter("tasktype", tasktype+"");
        params.addBodyParameter("state", state+"");
        params.addBodyParameter("tapNum", tapNum);
        params.addBodyParameter("netGateCode", netGateCode);
        params.addBodyParameter("maxAddTime",maxAddTime);
        params.addBodyParameter("minAddTime", minAddTime);
    ...
    public void netSuccess(String url, String result) {
        ...
        if(!TextUtils.isEmpty(url) && Const.TASKTOVIEW.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)){

            // 获取用户类型
            Type type= new TypeToken<CommonData<List<TaskToView>>>(){}.getType();
            LogUtil.e("任务详情", "解析前");
            CommonData<List<TaskToView>> datamessage = JsonParserUtil.getObject(result, type);

            LogUtil.e("任务详情", "解析后"+(datamessage.toString()));
            //判断解析数据是否为空
            if(datamessage!=null){
