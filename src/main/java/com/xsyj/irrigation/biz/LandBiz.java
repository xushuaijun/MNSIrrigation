 ...
 */
public class LandBiz extends AbstractNetBiz{

    private Context context;
    private NetCallBack<CommonData<List<Land>>> netcallback;
    ...
    public void land_getdata(Context context,String landname,String start_id,String page_size,NetCallBack<CommonData<List<Land>>> netcallback){
        ...
        RequestParams params=new RequestParams(Const.land_list);
        ...
        params.addBodyParameter("landname",landname);
    ...
    public void netSuccess(String url, String result) {
        ...
        if(!TextUtils.isEmpty(url) && Const.land_list.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)){

            // 获取用户类型
            Type type= new TypeToken<CommonData<List<Land>>>(){}.getType();
            LogUtil.e("地块信息列表", "解析前");
            CommonData<List<Land>> datamessage = JsonParserUtil.getObject(result, type);
