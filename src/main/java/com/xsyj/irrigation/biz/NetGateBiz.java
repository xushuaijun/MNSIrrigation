 ...
 */
public class NetGateBiz extends AbstractNetBiz{

    private Context context;
    private NetCallBack<CommonData<List<NetGate>>> netcallback;
    ...
    public void net_getdata(Context context,String netGateName,String start_id,String page_size,
                            NetCallBack<CommonData<List<NetGate>>> netcallback){
        ...
        params.addBodyParameter("netGateName",netGateName);
...
//        params.addBodyParameter("page_size", page_size);
    ...
    public void netSuccess(String url, String result) {
        LogUtil.e("网关信息列表", result);
        if(!TextUtils.isEmpty(url) && Const.NetGate_list.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)){

            // 获取用户类型
            Type type= new TypeToken<CommonData<List<NetGate>>>(){}.getType();
            LogUtil.e("阀门信息列表", "解析前");
            CommonData<List<NetGate>> datamessage = JsonParserUtil.getObject(result, type);
