 ...
 */
public class TimeTaskBiz extends AbstractNetBiz {
    ...
    public void get_timedata(Context context,long startTime,String tapNum,String openMouth,NetCallBack<CommonData<List<TaskResult>>> netcallback){
        ...
        RequestParams params=new RequestParams(Const.DSRW);
        ...
        params.addBodyParameter("openMouth",openMouth);
    ...
    public void netSuccess(String url, String result) {
        LogUtil.e("定时任务列表", result);
        if(!TextUtils.isEmpty(url) && Const.DSRW.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)){

            // 获取用户类型
            Type type= new TypeToken<CommonData<List<TaskResult>>>(){}.getType();
            LogUtil.e("定时任务列表", "解析前");
            CommonData<List<TaskResult>> datamessage = JsonParserUtil.getObject(result, type);

            LogUtil.e("定时任务列表", "解析后"+(datamessage.toString()));
            //判断解析数据是否为空
            if(datamessage!=null){
