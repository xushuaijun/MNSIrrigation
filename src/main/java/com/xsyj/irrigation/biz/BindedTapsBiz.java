 ...
 */
public class BindedTapsBiz extends AbstractNetBiz {


    private Context context;
    private NetCallBack<List<TapInfo>> netcallback;

    public BindedTapsBiz(){
        super();
     ...
     */
    public void getinfo(Context context,String turnId,NetCallBack<List<TapInfo>> netcallback){
        ...
        RequestParams params=new RequestParams(Const.binded_taps);
  ...
  /*      RequestParams params=new RequestParams("http://lifeplus.applinzi.com/exercise/statistics");
    ...
    public void netSuccess(String url, String result) {
        LogUtil.e("绑定的阀口列表信息", result);

        if(!TextUtils.isEmpty(url) && Const.binded_taps.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)){
