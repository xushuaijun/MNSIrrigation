 ...
 */
public class WellListBiz extends AbstractNetBiz {


    private Context context;
    private NetCallBack<List<WELL>> netcallback;
     ...
     */
    public void getinfo(Context context,NetCallBack<List<WELL>> netcallback){
        ...
        RequestParams params=new RequestParams(Const.well_list);
    ...
    public void netSuccess(String url, String result) {
        LogUtil.e("水源首部列表信息", result);

        if(!TextUtils.isEmpty(url) && Const.well_list.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)){



           // 获取用户类型
            Type type= new TypeToken<CommonData<List<WELL>>>(){}.getType();

            LogUtil.e("水源首部列表", "解析前");
            CommonData<List<WELL>> datamessage = JsonParserUtil.getObject(result, type);
           

            LogUtil.e("水源首部列表", "解析后"+(datamessage.toString()));
            //判断解析数据是否为空
            if(datamessage!=null){
