 ...
 */
public class TurnInfoListBiz extends AbstractNetBiz {


    private Context context;
    private NetCallBack<List<IrrigationTurnInfo>> netcallback;

    public TurnInfoListBiz(){
        super();
     ...
     */
    public void getinfo(Context context,NetCallBack<List<IrrigationTurnInfo>> netcallback){
        ...
        RequestParams params=new RequestParams(Const.turne_condition_list);
    ...
    public void netSuccess(String url, String result) {
        ...
        if(!TextUtils.isEmpty(url) && Const.turne_condition_list.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)){
           // 获取用户类型
