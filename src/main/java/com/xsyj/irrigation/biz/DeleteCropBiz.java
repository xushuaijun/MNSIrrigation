...
public class DeleteCropBiz extends AbstractNetBiz {


    private Context context;
    private NetCallBack<PileDeleteResult> netcallback;

    public DeleteCropBiz(){
        super();

    }

    public void delCrop(Context context,String theser ,
        ...
        RequestParams params=new RequestParams(Const.crop_delete);
        params.addBodyParameter("token", MyApplication.mApplication.getToken());
        params.addBodyParameter("theser",theser);
        httpbiz.connectNet(HttpBiz.POST, params);
    }
    @Override
    public void netSuccess(String url, String result) {
        if(!TextUtils.isEmpty(url) && Const.crop_delete.equalsIgnoreCase(url) && !TextUtils.isEmpty(result)) {
